...
const CuentasPagarRegistrationForm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const subcats = [
          'Materiales', 'Servicios', 'Equipos', 'Software', 'Hardware',
          'Consultoría', 'Mantenimiento', 'Capacitación', 'Marketing',
          'Transporte', 'Alimentación', 'Hospedaje', 'Otros'
        ];
  ...
  const handleProveedorChange = (proveedor) => {
    if (proveedor) {
      setCuenta(prev => ({
        ...prev,
        proveedor_id: proveedor.id,
        proveedor_nombre: proveedor.nombre,
        proveedor_rfc: proveedor.run_proveedor || '',
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (window.refreshCuentasPagar) {
        window.refreshCuentasPagar();
      }
            ...
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <StyledTextField
                  ...
                  placeholder="Ej: Compra de materiales para proyecto X"
                  InputProps={{
                    startAdornment: (
              ...
              <Grid item xs={12}>
                <StyledTextField
                  ...
                  multiline
                  rows={3}
                  placeholder="Descripción detallada de la cuenta por pagar..."
                />
              </Grid>
            ...
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <Autocomplete
                  ...
                  value={proveedores.find(p => p.id === cuenta.proveedor_id) || null}
                  onChange={(event, newValue) => handleProveedorChange(newValue)}
                  renderInput={(params) => (
                    <StyledTextField
                      {...params}
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                        ...
                        <Box>
                          ...
                          <Typography variant="caption" color="text.secondary">
                            {option.run_proveedor} • {option.elemento}
                          </Typography>
                        </Box>
                      </Box>
                    </Box>
                ...
                <Grid item xs={12}>
                  <Alert severity="info" sx={{ borderRadius: 2 }}>
                    <Typography variant="body2">
                      <strong>Proveedor seleccionado:</strong> {cuenta.proveedor_nombre}
                      {cuenta.proveedor_rfc && ` (${cuenta.proveedor_rfc})`}
                    </Typography>
                  </Alert>
                </Grid>
              )}
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} sm={6}>
                <StyledTextField
                  ...
                  InputLabelProps={{ shrink: true }}
                  helperText="Fecha límite para el pago (opcional)"
                />
              </Grid>
              ...
              <Grid item xs={12}>
                <Alert severity="info" sx={{ borderRadius: 2 }}>
                  <Typography variant="body2">
                    ...
                    Podrás marcar como pagada y agregar más detalles después desde la lista de cuentas.
                  </Typography>
                </Alert>
              </Grid>
            </Grid>
      ...
      <CardContent>
        <Box sx={{ mb: 4 }}>
          ...
          <Typography variant="body1" sx={{ textAlign: 'center', color: '#7f8c8d' }}>
            Complete todos los campos para registrar una nueva cuenta por pagar
          </Typography>
        </Box>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 4 }}>
          ...
          <Box sx={{ display: 'flex', gap: 2 }}>
            {activeStep === steps.length - 1 ? (
              ...
              >
                {loading ? 'Creando Cuenta...' : 'Crear Cuenta por Pagar'}
              </StyledButton>
            ) : (
              <StyledButton
                color="#667eea"
...
export default CuentasPagarRegistrationForm;