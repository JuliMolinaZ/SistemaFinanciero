...
// Variable de entorno para la URL base (si no existe, usar localhost)
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12}>
          ...
          <Button variant="outlined" component="label">
            Seleccionar Archivo
            <input type="file" name="facturaPDF" accept="application/pdf" hidden onChange={onChange} />
          </Button>
        </Grid>
        <Grid item xs={12}>
          <Typography variant="subtitle2" sx={{ mb: 0.5 }}>
          ...
          <Button variant="outlined" component="label">
            Seleccionar Archivo
            <input type="file" name="facturaXML" accept=".xml,.xlsx,.csv" hidden onChange={onChange} />
          </Button>
        </Grid>
      </Grid>
    </Box>
...
// =====================
const MovementsTable = ({ movimientos, handleEdit, handleDelete }) => (
...
const ContabilidadForm = () => {
  ...
  // Al cargar el componente, obtener movimientos
  useEffect(() => {
    fetchMovimientos();
  }, []);
  ...
  const handleChange = (e) => {
    ...
    if (name === 'facturaPDF' || name === 'facturaXML') {
      ...
      // Al cambiar de cargo a abono, limpiar los valores
  ...
  // Envío del formulario (registrar o actualizar)
  const handleSubmit = async (e) => {
    ...
    // Validación: debe existir solo un valor mayor a 0
      ...
      {loading ? (
        ...
        <>
          <FormDialog
            ...
            error={error}
          />
          <MovementsTable movimientos={movimientos} handleEdit={handleEdit} handleDelete={handleDelete} />
        </>
      )}
    </Container>
  );
